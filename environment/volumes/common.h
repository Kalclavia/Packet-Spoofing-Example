...
void send_raw_ip_packet(ipheader *ip)
{
    ...
    int sock_status = socket(AF_INET, SOCK_RAW, IPPROTO_RAW);
    if (sock_status == -1) {
    ...
    int sockopt_status = setsockopt(sock_status, IPPROTO_IP, IP_HDRINCL, &enable, sizeof(enable));
    if (sockopt_status < 0) {
        perror("Error with setsocketopt!");
        exit(1);
    }
    ...
    // DONE
     ...
     if (sendto(sock_status, ip, ntohs(ip->iph_len), 0, (struct sockaddr *)&dest_info, sizeof(dest_info)) < 0) {
